# Use singleton class Ecommerce::Preferences::Store.instance to access
#
...
module Ecommerce::Preferences
  class StoreInstance
    ...
    def exist?(key)
      @cache.exist?(key) ||
        should_persist? && Ecommerce::Preferences.where(key: key).exists?
    end
    ...
    def get(key)
      ...
      if should_persist?
        ...
        val = if preference = Ecommerce::Preferences.find_by(key: key)
          # it does exist
                preference.value
              else
    ...
    def persist(cache_key, value)
      ...
      preference = Ecommerce::Preferences.where(key: cache_key).first_or_initialize
    ...
    def destroy(cache_key)
      ...
      preference = Ecommerce::Preferences.find_by(key: cache_key)
      preference.destroy if preference
    end

    def should_persist?
      @persistence && ActiveRecord::Base.connected? && Ecommerce::Preference.table_exists?
    end
  end
