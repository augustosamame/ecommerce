  ...
  class Order < ApplicationRecord
    ...
    def generate_einvoice
      ...
      OrderItem.where(order_id: self.id).includes(:product).each do |item|
        invoice_lines_array << {name: item.product.name, quantity: item.quantity, product_id: item.product.id,  price_subtotal: ((item.price / 1.18).to_f), price_total: item.price.to_f * item.quantity, }
