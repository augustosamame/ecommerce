module Ecommerce
  ...
  module GlobalcanastaHelper
    ...
    # Backoffice links are often saved as absolute expatshop.pe URLs. Any of
    # our own hosts is rewritten to a path so the visitor stays on the domain
    # (and theme) they arrived on.
    OWN_HOSTS = /\A(www\.)?(expatshop\.pe|globalcanasta\.(com|pe)|globalcanasta\.localhost|localhost)\z/i

    def gc_local_url(url)
      uri = URI.parse(url.to_s.strip)
      return url unless uri.host && uri.host.match?(OWN_HOSTS)
    ...
    # Home hero copy: Backoffice → Controls (home_hero_<field>_<es|en>),
    # falling back to the other language, then to the locale files.
    def gc_hero_copy(field)
      en = I18n.locale.to_s.start_with?("en")
      order = en ? %w[en es] : %w[es en]
      value = order.map { |l| Ecommerce::Control.get_control_value("home_hero_#{field}_#{l}") }.find(&:present?)
      value.presence || t("ecommerce.globalcanasta.store.main.hero_#{field}")
    end
